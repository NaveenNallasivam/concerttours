...
export class ComponentHandlerService {
    ...
    getFirstSmartEditComponentChildren(htmlElement: HTMLElement | JQuery): JQuery[] {
        ...
        const firstChildrenRegex: RegExp = new RegExp(COMPONENT_CLASS, 'g');
        return stem.find('.' + COMPONENT_CLASS).filter((index: number, component: JQuery) => {
            const match: string = (this.yjQuery(component) as any)
                .getCssPath()
                .replace(parentCssPath, '')
                .match(firstChildrenRegex);
