...
describe('singleActiveCatalogAwareSelector', function() {
    ...
    beforeEach(
        angular.mock.module('pascalprecht.translate', function($translateProvider, $provide) {
    ...
    beforeEach(
        angular.mock.module('singleActiveCatalogAwareItemSelectorModule', function(
            $provide,
            $compileProvider
        ) {
            // Product Catalogs
    ...
    function renderCatalogAwareItemSelector(selectorType) {
        ...
        element = $compile(
            '<single-active-catalog-aware-item-selector ' +
                ...
                'data-editor="editor"></single-active-catalog-aware-item-selector>'
        )(scope);
        scope.$digest();
    }
    ...
    function assertItemSelectorWithRightPropertyTypeIsDisplayed(selectorCmsType) {
        ...
        expect(itemSelectorCtrl.field.propertyType).toBe(
            getPropertyTypeForCmsType(selectorCmsType)
        );
    }
    ...
    function assertItemTypeNotSupportedIsDisplayed() {
        ...
        expect(msgElement.text().trim()).toBe(ITEM_TYPE_MISSING);
    }
});
