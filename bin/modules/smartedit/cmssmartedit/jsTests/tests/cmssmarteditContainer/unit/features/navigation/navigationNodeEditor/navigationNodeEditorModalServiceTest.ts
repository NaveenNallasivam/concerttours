...
describe('NavigationNodeEditorModalService', () => {
    ...
    beforeEach(() => {
        genericEditorModalService = jasmine.createSpyObj<any>('genericEditorModalService', [
            'open'
        ]);
    ...
    it('should throw error if uri context parameter is missing', () => {
        expect(() => {
            ...
            new Error(
                `NavigationNodeEditorModalService.open : missing [uriContext : IUriContext] parameter.`
            )
        );
    });
    ...
    it('should throw error if parent node parameter is missing', () => {
        expect(() => {
            ...
            new Error(
                `NavigationNodeEditorModalService.open : missing [parent: NavigationalNode] parameter.`
            )
        );
    });
});
