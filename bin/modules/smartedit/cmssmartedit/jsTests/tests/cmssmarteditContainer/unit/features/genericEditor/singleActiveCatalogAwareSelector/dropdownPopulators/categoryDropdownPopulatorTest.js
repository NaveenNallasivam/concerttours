...
describe('categoryDropdownPopulator', function() {
    ...
    beforeEach(function() {
        expectedSearchUri = PRODUCT_CATEGORY_SEARCH_RESOURCE_URI.replace(
            /:catalogId/gi,
        ...
        var harness = AngularUnitTestHelper.prepareModule('categoryDropdownPopulatorModule')
            .mockConstant('extend', function(parent, child) {
                return child;
            })
            .mockConstant('DropdownPopulatorInterface', function() {})
            .mock('contextAwareCatalogService', 'getProductCategorySearchUri')
            .and.returnResolvedPromise(expectedSearchUri)
            .mock('contextAwareCatalogService', 'getProductCategoryItemUri')
            .and.returnResolvedPromise(expectedItemUri)
            .mock('uriDropdownPopulator', 'fetchPage')
            .and.returnResolvedPromise({
                productCategories: PRODUCT_CATEGORIES_MOCK
            })
            .mock('uriDropdownPopulator', 'getItem')
            .and.returnResolvedPromise(PRODUCT_CATEGORIES_MOCK[0])
