...
describe('productCatalogDropdownPopulator', function() {
    ...
    var expectedCatalogs = CATALOGS_MOCKS.filter(function(catalog) {
        return (
            catalog.versions.find(function(version) {
    ...
    beforeEach(function() {
        var harness = AngularUnitTestHelper.prepareModule('productCatalogDropdownPopulatorModule')
            ...
            .and.returnResolvedPromise(CATALOGS_MOCKS)
            .mockConstant('CONTEXT_SITE_ID', 'CONTEXT_SITE_ID')
            .mock('optionsDropdownPopulator', 'populate')
            .and.returnResolvedPromise(expectedCatalogs)
