...
import {
    PageInfoForViewing,
    PageInfoMenuService
} from 'cmssmarteditcontainer/components/pages/pageInfoMenu/services/PageInfoMenuService';
...
describe('PageInfoMenuService', () => {
    ...
    // Constants
    // ---------------------------------------------------------------------------
    const REJECTED_PROMISE = promiseHelper.buildPromise<any>(
        'rejectedPromise',
        PromiseType.REJECTS
    );
    ...
    beforeEach(() => {
        ...
        pageEditorModalService.open.and.returnValue(
            promiseHelper.buildPromise<string>('resolvedPromise', PromiseType.RESOLVES)
        );

        pageService = jasmine.createSpyObj('pageService', ['getCurrentPageInfoByVersion']);
        pageService.getCurrentPageInfoByVersion.and.returnValue(
            promiseHelper.buildPromise<any>('resolvedPromise', PromiseType.RESOLVES, pageInfo)
        );
        ...
        displayConditionsFacade.getPrimaryPageForVariationPage.and.returnValue(
            promiseHelper.buildPromise(
                'primaryPageForVariationPromise',
        ...
        typeStructureRestService.getStructureByType.and.returnValue(
            promiseHelper.buildPromise<CMSItemStructureField[]>(
                'resolvedPromise',
                PromiseType.RESOLVES,
                structureRetrieved
            )
        ...
        pageInfoMenuService = new PageInfoMenuService(
            pageService,
    ...
    it('GIVEN current page info cannot be retrieved WHEN getCurrentPageInfo is called THEN a warning message is displayed in the console', () => {
        // GIVEN
        pageService.getCurrentPageInfoByVersion.and.returnValue(REJECTED_PROMISE);
    ...
    it('GIVEN variation page WHEN getPageStructureForViewing is called THEN it returns the structure in the proper format for viewing', () => {
        ...
        const structureExpected: CMSItemStructureField[] = [
            ...
            buildStructureField('description', 'ShortString', false),
