 ...
 */
angular
    .module('selectTargetCatalogVersionModule', [
        'yLoDashModule',
        'pageFacadeModule',
    ...
    .controller('selectTargetCatalogVersionController', [
        ...
        function(
            ...
            this.catalogVersionSelectorFetchStrategy = {
                fetchAll: function() {
                    return $q.when(
                        lodash.forEach(this.catalogVersions, function(catalogVersion) {
            ...
            this.onSelectionChanged = function() {
                ...
                catalogService
                    .getCatalogVersionByUuid(this.selectedCatalogVersion)
                    .then(
                        function(catalogVersion) {
                            ...
                            if (this.pageTypeCode === 'ContentPage') {
                                return pageFacade.contentPageWithLabelExists(
                                    this.pageLabel,
                            ...
                            uriContextForSelectedCatalogVersion[PAGE_CONTEXT_SITE_ID] =
                                catalogVersion.siteId;
                            uriContextForSelectedCatalogVersion[PAGE_CONTEXT_CATALOG] =
                                catalogVersion.catalogId;
                            uriContextForSelectedCatalogVersion[PAGE_CONTEXT_CATALOG_VERSION] =
                                catalogVersion.version;

                            return pageService.getPrimaryPagesForPageType(
                                this.pageTypeCode,
                    ...
                    .then(
                        function(result) {
                            if (this.pageTypeCode === 'ContentPage') {
                                ...
                                this.catalogVersionContainsPageWithSameTypeCode = !lodash.isEmpty(
                                    result
                                );
                            }
            ...
            this.$onInit = function() {
                catalogVersionRestService.getCloneableTargets(this.uriContext).then(
                    function(targets) {
                        var catalogVersions = targets.versions;

                        if (!lodash.isEmpty(catalogVersions)) {
                            catalogService.getCatalogVersionUUid(this.uriContext).then(
                                function(uuid) {
                                    var currentCatalogVersion = lodash.find(
                                        catalogVersions,
                                        function(catalogVersion) {
