...
export class MissingPrimaryContentPageComponent implements ISeComponent {
    ...
    // Constructor
    // ------------------------------------------------------------------------
    constructor(private pageService: any, private $log: angular.ILogService) {}

    // ------------------------------------------------------------------------
    // Helper Methods
    // ------------------------------------------------------------------------
    private fetchAll(): angular.IPromise<ICMSPage[]> {
        return this.pageService.getPrimaryPagesForPageType(this.CONTENT_PAGE_TYPE_CODE).then(
            (rawPages: ICMSPage[]) => {
