 ...
 */
angular
    .module('displayConditionsEditorModule', [
    ...
    .controller('displayConditionsEditorController', function(
        ...
        this.$onInit = function() {
            ...
            pageService.buildUriContextForCurrentPage().then(
                function(uriContext) {
                    pageService.isPagePrimaryWithContext(this.page.uid, uriContext).then(
                        function(boolResult) {
                            this.isPrimaryPage = boolResult;
                        }.bind(this)
                    );
                    homepageService.hasFallbackHomePage(uriContext).then(
                        function(boolResult) {
                            this.hasFallback = boolResult;
                        }.bind(this)
                    );
                    homepageService.getHomepageDetailsForContext(uriContext).then(
                ...
                function() {
                    $log.error(
                        'displayConditionsEditorController::$onInit - unable to retrieve uriContext'
                    );
                }
            );
        };
        ...
        this.getHomePageDisabledTooltipTemplate = function() {
            return (
                '<span>' +
                $translate.instant('se.cms.display.conditions.homepage.disabled') +
                '</span>'
            );
        };
    })
