...
public class ConfigurationFacadeImplTest
{
	...
	public void setup()
	{
		...
		/*
		 * we use mock instead of real class, because calling baseStore.getName() would lead to illegalArgument Exception, as no
		 * LocalProvider is injected into the item Context. baseStore.getName() is used in some debug statetments, so when
		 * running tests with log level debug this would lead to an exception
		 */
